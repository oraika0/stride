            ...
            try:
                ...
                if self.link_free_bw_dir:
                    ...
                    for link in self.link_free_bw_dir:
                        self.net_info_directed[link] = [round(self.link_free_bw_dir[link],6), round(self.delay.link_delay.get(link,0.0),6), round(self.link_loss.get(link,0.0),6)]
                        self.net_metrics_directed[link] = [round(self.link_free_bw_dir[link],6), round(self.link_used_bw_dir.get(link,0.0),6), round(self.delay.link_delay.get(link,0.0),6), round(self.link_loss.get(link,0.0),6)]

                    with open(f"../results/{alg_name}/net_info_directed.csv",'w') as csvfile:
                        header_names = ['node1','node2','bwd','delay','pkloss']
                        file = csv.writer(csvfile, delimiter=',',quotechar='|', quoting=csv.QUOTE_MINIMAL)
                        file.writerow(header_names)
