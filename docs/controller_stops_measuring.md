...
[`../loader/train_loader.py`](../loader/train_loader.py) 的 `check_controller_alive`
在每一步讀完 state 之後執行。它對 `results/<alg>/net_info_directed.csv` 取 hash，數連
續幾次相同，達到五次就中止這次訓練。

這個判斷不依賴上面兩種失敗是哪一種，也不需要事先知道下一種是什麼。有流量在跑時，兩個
週期不可能寫出位元完全相同的檔案 —— 每條有向鏈路都貢獻剩餘頻寬、佇列延遲與封包遺失三
個數字，要全部同時落在同一組值上才會相同。相同就是 controller，不是網路。

| 參數 | 預設 | 意義 |
| --- | --- | --- |
| `stall_abort_steps` | 5 | 連續幾次相同就中止；設 0 停用 |
| `stall_grace_steps` | 10 | 開頭跳過的步數 —— 流量還沒起來時，閒置的網路確實會寫出相同的檔案 |
