...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, keyvalue):
        ...
        # fused scaled-dot-product attention (mem-efficient / flash backend): does
        # NOT materialize the (bs*na, heads, sl, sl) score matrix nor store it for
        # backward -- that matrix is the memory hog on 32node where na=num_agents=992
        # sits in the batch dim (Geant 506 fit; 32node OOM'd 2026-06-04, run sod6e60s).
        # Same math as the old matmul/softmax/matmul; sdpa's default scale
        # 1/sqrt(head_dim) == the old / (head_dim ** 0.5). Q/K/V already
        # (bs*na, num_heads, sl, head_dim) = sdpa's expected (batch, heads, L, E).
...
class ls2ic_agent():
    ...
    def append_sample(self, info, next_state, reward, next_link_state_raw=None):
        # next_link_state_raw is a leftover of the removed candidate-feature heads;
        # nothing passes it any more. ls2ic stores its own (state, actions, logits,
        # next_state, reward) and has no use for the raw link state, so it is
        # accepted and ignored (matches stride's
        # signature so the unconditional train_loader call no longer TypeErrors).
