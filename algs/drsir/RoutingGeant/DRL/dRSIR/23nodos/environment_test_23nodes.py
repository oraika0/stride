...
class Environment(object):
    def __init__(self, alg_name="drsir"):
        self.alg_name = alg_name  # results dir for paths_metrics.json (drsir / drsir_dd)
    ...
    def path_metrics_to_reward(self):
        ...
        # ini = time.time()
        file = f'./results/{self.alg_name}/paths_metrics.json'
        num_actions = self.act_space_size
        rewards_dic = {}
        metrics = ['bwd_paths','delay_paths','loss_paths']


        # 2026-07-11: retry-guard the read -- the controller rewrites this file
        # every MONITOR_PERIOD and a mid-write read raises JSONDecodeError. This
        # killed DRL_thread at iteration 5 of pc0 geant s17 TM03 (25 iterations
        # lost). Mirrors simple_monitor.get_dRL_paths' retry (0.35 s backoff).
        last_err = None
        for _attempt in range(3):
            try:
                ...
                last_err = e
                print(f"[drsir env] paths_metrics.json read failed "
