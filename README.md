...
Both CSVs are rebuilt from the `output_*.txt` files under
`results/*/runs/*/train/`, which are the original record of every training run.
**You do not run `make_curves_csv.py` first** — each of the two figure scripts
calls it on the way past. The rebuild costs a fraction of a second, which is
cheaper than deciding whether it is needed, and it removes the quieter failure
where a figure plots a CSV older than the runs it claims to show and says
nothing.

The only thing to edit is `RUN_MAP` in `make_curves_csv.py`, when a run is
added. That script still runs standalone and still writes both CSVs. Nothing in
`paper/` needs network access.
